...
def _is_office_preview_file(path: Path) -> bool:
    return path.suffix.lower() in OFFICE_PREVIEW_EXTENSIONS


def _office_preview_path(path: Path) -> Path:
    return path.with_suffix(f"{path.suffix}.preview.pdf")


def _preview_is_fresh(source: Path, preview: Path) -> bool:
    return preview.exists() and preview.stat().st_mtime >= source.stat().st_mtime


def _ensure_office_pdf_preview(source_path: Path) -> Path:
    if not _is_office_preview_file(source_path):
        raise ValueError(f"Preview conversion only supports office files: {source_path}")

    preview_path = _office_preview_path(source_path)
    if _preview_is_fresh(source_path, preview_path):
        return preview_path

    preview_path.parent.mkdir(parents=True, exist_ok=True)
    source_name = source_path.name

    with tempfile.TemporaryDirectory(prefix=f"{source_path.stem}-", dir=str(preview_path.parent)) as tmp_dir:
        ...
        if result.returncode != 0:
            logger.error(
                "Office preview conversion failed for %s: stdout=%s stderr=%s",
                source_name,
                result.stdout.strip(),
                result.stderr.strip(),
            )
            raise RuntimeError(f"Failed to convert {source_name} to PDF preview")

        generated_pdf = tmp_dir_path / f"{source_path.stem}.pdf"
        if not generated_pdf.exists():
            raise RuntimeError(f"Converted preview missing for {source_name}")
...
async def get_artifact(
    ...
    if ".skill/" in path:
        ...
        actual_skill_path = resolve_thread_virtual_path(thread_id, skill_file_path, user_id=x_user_id)
    ...
    actual_path = resolve_thread_virtual_path(thread_id, path, user_id=x_user_id)
    ...
    if request.query_params.get("preview") == "pdf" and _is_office_preview_file(actual_path):
        try:
            preview_path = _ensure_office_pdf_preview(actual_path)
        except RuntimeError as exc:
            raise HTTPException(status_code=500, detail=str(exc)) from exc

        encoded_preview_name = quote(preview_path.name)
        return FileResponse(
            path=preview_path,
            media_type="application/pdf",
            headers={"Content-Disposition": f"inline; filename*=UTF-8''{encoded_preview_name}"},
        )
