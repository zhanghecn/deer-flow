...
def save_skill_to_store(
    ...
    try:
        ...
        resolved_source = (
            ...
            if source_path
            else resolve_default_skill_source_dir(runtime=runtime, skill_name=skill_name, paths=paths)
        )
        ...
        parts = [f"Skill '{skill_name}' saved to {target_dir}."]
        ...
        logger.error("Failed to save skill '%s': %s", skill_name, exc, exc_info=True)
