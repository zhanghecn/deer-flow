...
def push_skill_prod(
    ...
    try:
        target_dir, backup_dir = push_skill_directory_to_prod(skill_name, paths=get_paths())
        parts = [f"Skill '{skill_name}' pushed to {target_dir}."]
        ...
        logger.error("Failed to push skill '%s' to prod: %s", skill_name, exc, exc_info=True)
