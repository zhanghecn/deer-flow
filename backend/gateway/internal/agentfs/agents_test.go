...
func TestPublishAgentRejectsDevScopedSkills(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(baseDir, "agents", "dev", "analyst", "skills", "research"), 0o755); err != nil {
		t.Fatalf("mkdir agent skills: %v", err)
	}
	if err := os.WriteFile(
		filepath.Join(baseDir, "agents", "dev", "analyst", "skills", "research", "SKILL.md"),
...
func TestPublishAgentAllowsAgentOwnedPrivateSkills(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(baseDir, "agents", "prod", "analyst", "skills", "research", "SKILL.md")); err != nil {
