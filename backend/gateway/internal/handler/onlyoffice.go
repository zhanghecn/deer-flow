...
func (h *OnlyOfficeHandler) Config(c *gin.Context) {
	...
	fileURL := fmt.Sprintf("%s/api/office/threads/%s/files/%s?user_id=%s", baseURL, url.PathEscape(threadID), fileRoutePath, userID.String())
	callbackURL := fmt.Sprintf("%s/api/office/threads/%s/callback/%s?user_id=%s", baseURL, url.PathEscape(threadID), fileRoutePath, userID.String())
...
func (h *OnlyOfficeHandler) File(c *gin.Context) {
	...
	userID := onlyOfficeUserIDFromRequest(c, claims)
	if userID == "" {
...
func onlyOfficeUserIDFromRequest(c *gin.Context, claims jwtv5.MapClaims) string {
	if userID := onlyOfficeUserIDFromClaims(claims); userID != "" {
		return userID
	}

	// ONLYOFFICE status/download callbacks can carry server-generated JWTs that
	// no longer include the original editorConfig block. The user_id query value
	// is generated by Gateway in Config and is only trusted after the office JWT
	// signature has been validated by authorizeOnlyOfficeRequest.
	if userID := strings.TrimSpace(c.Query("user_id")); isUUIDString(userID) {
		return userID
	}
	return ""
}
...
func onlyOfficeUserIDFromClaims(claims jwtv5.MapClaims) string {
	editorConfig, ok := claims["editorConfig"].(map[string]any)
	if !ok {
		return ""
	}
	user, ok := editorConfig["user"].(map[string]any)
	if !ok {
		return ""
	}
