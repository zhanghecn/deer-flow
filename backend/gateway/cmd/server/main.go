...
func main() {
	...
	api.Use(middleware.JWTAuth(jwtMgr, userRepo))
	{
		...
		api.POST("/skills/import", skillH.Import)
		...
		api.GET("/knowledge/bases/:knowledge_base_id/workspace/tree", knowledgeH.WorkspaceTree)
		api.GET("/knowledge/bases/:knowledge_base_id/workspace/file", knowledgeH.WorkspaceFile)
		...
		admin.Use(middleware.AdminOnly())
		{
			...
			admin.GET("/users/:id/tokens", authH.ListTokens)
			admin.POST("/users/:id/tokens", authH.CreateToken)
			admin.DELETE("/users/:id/tokens/:token_id", authH.DeleteToken)
			admin.POST("/agents/import", agentH.ImportPackageAdmin)
			admin.PATCH("/agents/:name/owner", agentH.SetOwnerAdmin)
