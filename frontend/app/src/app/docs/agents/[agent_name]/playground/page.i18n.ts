...
const enUS: AgentPublicPlaygroundPageText = {
  ...
  eyebrow: "Playground",
  title: "Run real public-agent calls without leaving the docs flow",
  description:
    "Use this page only for live execution: blocking requests, SSE, file upload, normalized run timelines, and generated artifact download. The overview and API reference stay separate so this page can stay focused and compact.",
  ...
  heroFactThreeValue: "The runtime panel stays in docs, but it no longer needs to dominate the whole page.",
...
const zhCN: AgentPublicPlaygroundPageText = {
  ...
  openHome: "打开工作台",
  eyebrow: "Playground",
  title: "在文档流里直接运行公开智能体调用",
  description:
    "这个页面只用于真实执行：blocking 请求、SSE、文件上传、归一化运行时间线和产物下载。概览和 API Reference 独立存在，这里只保留必要的运行契约调试能力。",
  ...
  heroFactThreeValue: "执行区保留在文档体系里，但不会再把整个页面做成夸张的大控制台。",
  stepsEyebrow: "使用流程",
  stepsTitle: "如何使用这个页面",
  stepsDescription:
