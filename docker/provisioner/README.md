...
The provisioner is configured via environment variables (set in [docker-compose.yaml](../docker-compose.yaml)):
...
docker compose --env-file .env -p openagents -f docker/docker-compose.yaml --profile provisioner up -d provisioner
```
...
docker compose -p openagents -f docker/docker-compose.yaml exec provisioner curl -X POST http://localhost:8002/api/sandboxes \
  -H "Content-Type: application/json" \
  -d '{"sandbox_id":"test-001","thread_id":"thread-001"}'

# Check sandbox status
docker compose -p openagents -f docker/docker-compose.yaml exec provisioner curl http://localhost:8002/api/sandboxes/test-001

# List all sandboxes
docker compose -p openagents -f docker/docker-compose.yaml exec provisioner curl http://localhost:8002/api/sandboxes

# Verify Pod and Service in K8s
kubectl get pod,svc -n openagents -l sandbox-id=test-001

# Delete sandbox
docker compose -p openagents -f docker/docker-compose.yaml exec provisioner curl -X DELETE http://localhost:8002/api/sandboxes/test-001
```
...
SANDBOX_URL=$(docker compose -p openagents -f docker/docker-compose.yaml exec -T provisioner curl -s http://localhost:8002/api/sandboxes/test-001 | jq -r .sandbox_url)

# Test from gateway container
docker compose -p openagents -f docker/docker-compose.yaml exec gateway curl -s $SANDBOX_URL/v1/sandbox
```
...
- Check the volume mount in docker-compose.yaml
...
- Verify inside container:
  ```bash
  docker compose -p openagents -f docker/docker-compose.yaml exec provisioner ls -ld /root/.kube/config
  ```
...
- Test from a backend container using the configured address:
  `docker compose -p openagents -f docker/docker-compose.yaml exec gateway curl -s http://$NODE_HOST:NODE_PORT/v1/sandbox`
