...
- Skill discovery is local-store-first: inspect `/mnt/skills/store/dev/...` and `/mnt/skills/store/prod/...` before considering any external registry search or installation.
...
- Do not edit archived store skills in place for domain agents. Reusable store skills must be attached via copied `skill_refs`.
