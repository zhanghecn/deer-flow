...
- Search the local archived skill library first.
- The canonical local archived library lives under `/mnt/skills/system/skills/...` and `/mnt/skills/custom/skills/...`.
...
- When a local skill is selected for agent creation, keep its exact `source_path` such as `system/skills/find-skills` or `custom/skills/pptx-generator`.
...
Inspect the local archived library before doing anything external:
...
- `system/skills/find-skills`
- `system/skills/frontend-design`
- `custom/skills/pptx-generator`
...
4. Whether it already exists in `system/skills` or `custom/skills`
...
```text
setup_agent(..., skills=[{source_path: "system/skills/find-skills"}])
```

If the same skill name exists in multiple authored roots, do not collapse it to a bare `{name}` entry.
