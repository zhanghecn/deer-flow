...
之前你对齐了一版,但是现在还是有一个问题。
1. read 总是只读100行的样子,总是读不全。
2. write 写入有时候会被截取,写不全。 或者有时候会并发调用 write 工具 导致失败,因为存在的文件只能通过edit
3. 执行命令感觉很慢,而且目前拥有 bash subagent 和 后端协议的执行命令感觉混乱。 
以上问题在opencode 是好的,说明没有对齐好。
注意识别 后端协议,目前用 local/sandbox 区分,不要改坏了。 你需要借助langchain 了解 deepagent 


现在 有头模式 打开测试 http://localhost:3000 。
然后点击 surprise 测试流程。
其中会生产一个可预览的html。
在完成后继续追加生产 ppt

你会发现下面问题:
1. 特别卡顿,原因不明,与manus claude 官网体验的效果差异很大
2. 当智能体执行20分钟后,其中 history 特别大,导致前端以为超时自己断开了,又重新链接。
3. skill 使用过程中可能有些涉及到执行脚本,里面需要环境变量里面的key,比如生成图片需要借助 GEMINI_API_KEY,但是 命令执行错误原因不知。 环境变量如何设置也不知。

找出最靠谱的办法进行修复

现在实际让你测试带思考的卡顿程度。是页面的问题。还是大模型产出的事件庞大 你可以切换 kimi 这个模型要快点。
然后现在还有一个问题就是 如何修改思考等级,对于 anthorpic 和 openai 格式 思考等级设置是不一样。 我想开启思考默认等级就最大。 包括max_token 建议是多少。 langchain 有什么规范吗。
如果修改的话是否能保证消息不丢失的情况下,保留历史记录进行优化。让用户永远保持流畅
## 后端协议方面
